  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/api/collections" && method === "POST") {
        const body = await request.json();
        const { title, slug, description, image_url, order_index, is_visible, title_it, description_it, detailed_description_it } = body;
        const finalTitle = title_it || title;
        const finalDescription = description_it || description;
        if (!finalTitle) {
      ...
      if (path === "/api/exhibitions" && method === "POST") {
        const body = await request.json();
        const { title, subtitle, location, date, description, info, website, image_url, slug, order_index, is_visible, title_it, subtitle_it, location_it, date_it, description_it, info_it } = body;
        const finalTitle = title_it || title;
        const finalSubtitle = subtitle_it || subtitle;
        const finalLocation = location_it || location;
        const finalDate = date_it || date;
        const finalDescription = description_it || description;
        const finalInfo = info_it || info;
        if (!finalTitle || !finalLocation || !finalDate) {
          return jsonResponse({ error: "Title, location and date are required" }, 400);
        }
        let finalSlug = slug;
        if (!finalSlug) {
          const yearMatch = finalDate.match(/\d{4}/);
          const year = yearMatch ? yearMatch[0] : (/* @__PURE__ */ new Date()).getFullYear().toString();
          finalSlug = generateSlug(finalTitle, year);
        }
        const result = await env.DB.prepare(
      ...
      if (path === "/api/critics" && method === "POST") {
        const body = await request.json();
        const { name, role, text, name_it, role_it, text_it, order_index, is_visible } = body;
        const finalName = name_it || name;
        const finalRole = role_it || role;
        const finalText = text_it || text;
        if (!finalName || !finalRole || !finalText) {
      ...
      if (path === "/api/detect-language" && method === "GET") {
...
// .wrangler/tmp/bundle-VOdKCb/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-VOdKCb/middleware-loader.entry.ts
