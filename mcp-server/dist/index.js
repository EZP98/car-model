    ...
    try {
        if (name === 'list_items_missing_translations') {
            ...
            if (type === 'exhibitions' || type === 'all') {
                const data = await apiCall('/api/exhibitions');
                results.exhibitions = data.exhibitions.map((item) => ({
                    id: item.id,
                    title: item.title,
                    date: item.date,
            ...
            if (type === 'collections' || type === 'all') {
                const data = await apiCall('/api/collections?all=true');
                results.collections = data.collections.map((item) => ({
                    id: item.id,
                    title: item.title,
                    slug: item.slug,
            ...
            if (type === 'critics' || type === 'all') {
                const data = await apiCall('/api/critics');
                results.critics = data.critics.map((item) => ({
                    id: item.id,
                    name: item.name,
                    role: item.role,
        ...
        if (name === 'get_item_for_translation') {
            ...
            const item = data.exhibition || data.collection || data.critic;
        ...
        if (name === 'update_item_translations') {
            const { type, id, translations } = args;
            let endpoint;
            if (type === 'exhibition') {
                endpoint = `/api/exhibitions/${id}`;
            }
...
// Helper functions
function getMissingTranslations(item, type) {
...
function extractSourceContent(item, type) {
    const content = {};
    if (type === 'exhibition') {
